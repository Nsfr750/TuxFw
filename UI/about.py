...
class AboutDialog(QDialog):
    """About dialog with logo display"""
    ...
    def __init__(self, parent, current_language):
        ...
        self.language_manager.set_language(current_language)
        
        # Set window title with translation
        self.setWindowTitle(self.language_manager.get_text('about.window_title', 'About'))
    ...
    def init_ui(self):
        ...
        # Application title
        title = QLabel(self.language_manager.get_text("about.title", "Firewall"))
        title.setStyleSheet("""
            font-size: 24px; 
            font-weight: bold;
            color: yellow;
        ...
        # Version information
        try:
            # Use the version from the imported module
            ...
            try:
                ...
                logger.debug(f"Could not import version utilities: {e}")
            except Exception as e:
        ...
        sys_info_title = QLabel(
            self.language_manager.get_text("about.system_info", "<h3>System Information</h3>")
        )
        ...
        # Close button
        close_btn = QPushButton(self.language_manager.get_text("about.close", "Close"))
    ...
    def retranslate_ui(self, language_code=None):
        ...
        try:
            ...
            if hasattr(self, 'version_label'):
                try:
                    version = __version__
                    codename = get_codename() if 'get_codename' in globals() and callable(get_codename) else ''
                    status = "Development" if 'is_development' in globals() and callable(is_development) and is_development() else "Stable"
                    version_format = self.language_manager.get_text(
                        "about.version", 
                        "Version {version}{codename} ({status})"
                    ).format(
                        version=version,
                        codename=f' {codename}' if codename and codename != 'unknown' else '',
                        status=status
                    )
